...
PRICES = {
    ...
    # grok-4.3 is DELIBERATELY ABSENT.
    #
    # The build-time search found grok-4.1 and grok-4.5 and no rate for 4.3. Interpolating between
    # neighbours, or borrowing 4.1's $0.20/$0.50, would be inventing a number and printing it next
    # to real ones -- precisely the liability this whole architecture exists to remove. Stage 3
    # therefore renders [UNVERIFIED] and is excluded from the total, and the report names it.
    # Fix = add the entry with a vendor-page source.
}
...
def _normalise_xai(j):
    ...
    return {"input": _n(u.get("input_tokens")) or _n(u.get("prompt_tokens")),
