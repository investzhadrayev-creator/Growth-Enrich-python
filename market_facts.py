...
def market_facts(ticker, peers=None, av_key=None, finnhub_key=None, tiingo_token=None,
    ...
    # ready-made ratio cannot pass unexamined; the reported one is kept for cross-check.
    ...
    if av and av.get("eps_ttm") and price:
        ...
        if fwd_pe_computed is None and av.get("forward_pe_reported"):
            out["fwd_pe_computed"] = av["forward_pe_reported"]
            out["fwd_pe_computed_basis"] = "AV ForwardPE reported (no growth estimate to compute our own)"
    ...
    if finra_client_id and finra_client_secret:
        ...
        if si:
            if shares_outstanding and si.get("short_shares"):
                si["short_pct_shares_outstanding"] = round(
                    si["short_shares"] / shares_outstanding * 100, 2)
