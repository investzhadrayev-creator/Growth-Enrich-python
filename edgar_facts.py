...
def _ratio_continuity(num_series, den_series, tol=0.30):
    """v4.2.46 (mandate BBB, corrected after the NFLX check).

    Detects a series discontinuity WITHOUT relying on tag semantics or on picking by magnitude —
    both disproved by the MA case: the emitter labelled its GROSS revenue with the ASC-606 tag, so
    semantic priority is unreliable, and choosing by size is the same class of error as
    min()-over-windows.

    A paired ratio (operating margin) is the right SIGNAL, but a threshold alone is NOT a criterion:
    measured on real data, MA's tag-switch moved the margin 63%, while NFLX moved it 88% (2012
    international expansion) and 277% (2013 recovery) with a SINGLE tag throughout. No threshold
    separates the two — a real business can out-jump a data defect.

    So the jump is only a DEFECT when it coincides with a TAG BOUNDARY. Jumps inside a
    single-tag stretch are business events and are returned separately, for information, never as
    defects. `den_series` points carry `tag` (v4.2.45); without it every jump is informational.
        ...
        if prev is not None and prev["ratio"] > 0 and ratio > 0:
            ...
            if jump > tol:
                rec = {"end": end, "prev_end": prev["end"],
                       ...
                       "tag": d.get("tag"), "prev_tag": prev["tag"]}
                tag_switch = (d.get("tag") is not None and prev["tag"] is not None
                              and d.get("tag") != prev["tag"])
                (defects if tag_switch else events).append(rec)
        prev = {"end": end, "ratio": ratio, "tag": d.get("tag")}
...
def edgar_facts(ticker=None, cik=None):
    ...
    if cont["defects"]:
        out["_flags"]["series_discontinuity_at_tag_switch"] = cont["defects"]
