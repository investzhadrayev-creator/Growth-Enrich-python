...
def _ratio_continuity(num_series, den_series, tol=0.30):
    """v4.2.45 (mandate BBB, replaces the rejected 'take the smaller value' rule).

    Detects a series discontinuity WITHOUT relying on tag semantics or on picking by magnitude —
    both of which the MA case disproved: the emitter labelled its GROSS revenue with the ASC-606
    tag, so semantic priority is unreliable, and choosing by size is the same class of error as
    min()-over-windows (it would have worked here only by luck).

    Instead it uses what actually CAUGHT the defect: a paired ratio. Operating margin
    (op_income / revenue) is stable year over year for a real business; a tag switch in the
    DENOMINATOR moves it by ~1.5x in one step. Any year whose ratio jumps by more than `tol`
    against the previous year is reported. Works for revenue<->operating_income and generalises to
    net income, shares, and anything else with a stable companion.

    Returns [{"end", "ratio", "prev_ratio", "jump_pct"}] — empty when the series is smooth.
    """
    num = {r["end"]: r["val"] for r in (num_series or []) if r.get("val")}
    den = {r["end"]: r["val"] for r in (den_series or []) if r.get("val")}
    ends = sorted(set(num) & set(den))
    breaks, prev = [], None
    for end in ends:
        if not den[end]:
            continue
        ratio = num[end] / den[end]
        if prev is not None and prev[1] > 0 and ratio > 0:
            jump = abs(ratio - prev[1]) / prev[1]
            if jump > tol:
                breaks.append({"end": end, "prev_end": prev[0],
                               "ratio": round(ratio, 4), "prev_ratio": round(prev[1], 4),
...
def edgar_facts(ticker=None, cik=None):
    ...
    cont = _ratio_continuity(out.get("operating_income"), out.get("revenue"))
    if cont:
        out["_flags"]["series_discontinuity_op_margin"] = cont
    if mixed_tag_map:
